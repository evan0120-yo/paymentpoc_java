...
public class PollingTaskScheduler {
        ...
        for (TaskProcessor<?> processor : processors) {
            for (String taskType : processor.getSupportedTypes()) {
                ...
                if (this.processorMap.containsKey(taskType)) {
                    System.err.println("WARNING: Duplicate TaskProcessor mapping for type '" + taskType + "'. Overwriting.");
                }
	...
	public void processOutboxEvents() {
		...
		for (PollingTaskRepository repo : taskRepositories) {
            ...
            if (claimedCount > 0) {
            	// 找到自己認領的outbox
                List<? extends PollingTask> tasksToProcess = repo.findByInstanceIdAndStatus(instanceId, processingStatus);
